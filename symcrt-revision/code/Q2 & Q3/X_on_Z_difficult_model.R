...
parameter_grid <- data.frame(
  d = seq(100, 300, by = 50),  # dimension of Z
  s_x = 5,                     # number of nonzero coefficients for X|Z model
  n = 100,                     # sample size
  s_y = 5                      # number of nonzero coefficients for X|Z model
)
...
ggsave(filename = "varying-dimension-binomial-gaussian.png",
       plot = type_I_err,
       device = "png",
       width = 5,
       height = 5)
...
saveRDS(check_results, 
        "~/Dropbox (Penn)/CRT-revision/symcrt-revision/code/result/Q2.rds")
