...
# normalize dCRT from X
dCRT_X_normalize <- function(data, B_small = 1000){
  X <- data$X
  Y <- data$Y
  Z <- data$Z
  n <- length(X)
  ...
  z_stat <- 1 / (sqrt(n) * GCM_sd) * sum(R)
  null_z_stats <- numeric(B_small)
  for(b in 1:B_small){
    X_resampled <- rnorm(n = n, mean = X_on_Z_fit$fitted.values, sd = 1)
    R_resampled <- (X_resampled - X_on_Z_predict)*(Y - Y_on_Z_predict)
    S_hat <- sd(R_resampled)
    null_z_stats[b] <- sum(1/sqrt(n)*sum(R_resampled))/S_hat
  }
  null_z_stats <- c(z_stat, null_z_stats)
  list(p_value <- 2*min(mean(null_z_stats >= z_stat), mean(null_z_stats <= z_stat)))
}
...
# normalized dCRT from Y
dCRT_Y_normalize <- function(data, B_small = 1000){
  X <- data$X
  Y <- data$Y
  Z <- data$Z
  n <- length(X)
  ...
  for(b in 1:B_small){
    ...
    R_resampled <- (X - X_on_Z_predict)*(Y_resampled - Y_on_Z_predict)
    S_hat <- sd(R_resampled)
    null_z_stats[b] <- 1 / (sqrt(n) * S_hat) * sum(R_resampled)
  }
  null_z_stats <- c(z_stat, null_z_stats)
  list(p_value <- 2*min(mean(null_z_stats >= z_stat), mean(null_z_stats <= z_stat)))
}
...
dCRT_X_normalize_spec_f <- simulatr_function(f = dCRT_X_normalize, arg_names = character(0), loop = TRUE)
dCRT_Y_normalize_spec_f <- simulatr_function(f = dCRT_Y_normalize, arg_names = character(0), loop = TRUE)


run_method_functions <- list(gcm_test = gcm_test_spec_f,
                             dCRT_X = dCRT_X_spec_f,
                             dCRT_Y = dCRT_Y_spec_f,
                             dCRT_X_normalize = dCRT_X_normalize_spec_f,
                             dCRT_Y_normalize = dCRT_Y_normalize_spec_f)
