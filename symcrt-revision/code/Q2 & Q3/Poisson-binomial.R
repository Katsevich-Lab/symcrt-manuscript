...
generate_data_f <- function(n, d, ground_truth){
  ...
  X <- rbinom(n = n, 1, prob = exp(rep(-3, n) + Z%*%ground_truth$gamma) / (1 + exp(rep(-3, n)  + Z%*%ground_truth$gamma)))
...
gcm_test <- function(data){
  ...
  X_on_Z_fit <- suppressWarnings(glm(X ~ Z, family = "binomial"))
...
dCRT_X <- function(data, B_small = 1000){
  ...
  for(b in 1:B_small){
    X_resampled <- rbinom(n = n, 1, prob = X_on_Z_fit$fitted.values)
...
type_I_err <- check_results$metrics |>
  ...
  labs(x = "Dimension of covariate Z",
       y = "Type I Error",
       title = "Binomial-Poisson model") +
  theme(legend.position = "bottom") 


ggsave(filename = "varying-dimension-binomial-poisson.png",
       plot = type_I_err,
       device = "png",
       width = 5,
       height = 5)
...
# save the rds result
saveRDS(check_results, "Q2_binomial_poisson.rds")
