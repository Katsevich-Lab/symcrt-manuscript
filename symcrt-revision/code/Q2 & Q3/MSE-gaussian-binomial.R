...
generate_data_f <- function(n, d, ground_truth){
  ...
  Y <- rbinom(n = n, 1, prob = 1/ (1 + exp(- rep(-3, n) -Z%*%ground_truth$beta)))
  oracle_Y <- 1/ (1 + exp(- rep(-3, n) -Z%*%ground_truth$beta))
...
MSE_X <- function(data, ground_truth){
  X <- data$X
  Z <- data$Z
...
# GCM test with lasso methods
MSE_Y <- function(data, ground_truth){
  Y <- data$Y
  Z <- data$Z
  oracle_Y <- data$oracle_Y
  n <- length(data$X)
  # run glm result
  Y_on_Z_fit <- suppressWarnings(glm(Y ~ Z, family = "binomial"))
...
MSE_X_spec_f <- simulatr_function(f = MSE_X, arg_names = character(0), loop = TRUE)
MSE_Y_spec_f <- simulatr_function(f = MSE_Y, arg_names = character(0), loop = TRUE)


run_method_functions <- list(MSE_X = MSE_X_spec_f,
                             MSE_Y = MSE_Y_spec_f)
...
MSE <- check_results$metrics |>
  ...
  labs(x = "Dimension of covariate Z",
       y = "MSE",
       title = "Gaussian-Binomial model") +
  theme(legend.position = "bottom") 


ggsave(filename = "MSE-gaussian-binomial.png",
       plot = MSE,
       device = "png",
       width = 5,
       height = 5)
