...
generate_data_f <- function(n, d, ground_truth){
  ...
  Y <- rbinom(n = n, 1, prob = 1/ (1 + exp(- rep(-3, n) -Z%*%ground_truth$gamma)))
...
dCRT_X <- function(data, B_small = 1000){
  ...
  gamma_glm <- as.vector(coef(X_on_Z_fit))[-1]
  for(b in 1:B_small){
    X_resampled <- rnorm(n = n, mean = Z%*%gamma_glm, sd = 1)
...
dCRT_Y <- function(data, B_small = 1000){
  ...
  null_z_stats <- numeric(B_small)
  for(b in 1:B_small){
    Y_resampled <- rbinom(n = n, 1, prob = Y_on_Z_fit$fitted.values)
...
ggsave(filename = "varying-dimension-gaussian-binomial.png",
       plot = type_I_err,
       device = "png",
       width = 5,
       height = 5)
...
# save the rds result
saveRDS(check_results, "Q2_gaussian_binomial.rds")
