...
figure_path <- "figures/type_I_Err_inflation_comparison.pdf"
if(file.exists(figure_path)){
  # if figure already exists, don't recompute it
  cat("Figure 2 already exists!\n")
} else{
  ...
  Li <- readRDS("simulation-results/confounding/Li.rds")
  Liu <- read_csv("simulation-results/confounding/Liu.csv", show_col_types = FALSE)
  rd_Candès <- read_csv("simulation-results/confounding/Candès.csv", show_col_types = FALSE)
  ...
  dotplot <- simulation_data %>%
    ...
    geom_hline(yintercept = typeI_err(Li, 0.05), color = "dodgerblue") +
