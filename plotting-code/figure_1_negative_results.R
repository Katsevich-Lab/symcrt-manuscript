...
figure_path <- "figures/negative_result_dCRT.pdf"
if(file.exists(figure_path)){
  # if figure already exists, don't recompute it
  cat("Figure 1 already exists!\n")
} else{
  ...
  pn1 <- type_I_err[[1]] |>
    ...
    ggplot(aes(x = nu, y = type_I_err, linetype = Y_on_Z_reg)) +
    geom_point() +
    geom_line(aes(linetype = Y_on_Z_reg)) +
