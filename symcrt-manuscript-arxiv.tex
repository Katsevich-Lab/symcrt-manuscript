...
\begin{document}
	...
	\begin{proposition} \label{prop:sufficient-for-variance-consistency} \linktoproof{proof:prop:sufficient-for-variance-consistency}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				\item $\V_{\law_n}[\prx|\prz] = f(\mu_{n,x}(\prz))$ for $f$ being Lipschitz on domain $\cup_{n=1}^{\infty}\mathrm{Conv}(\law_{n}(\prx))$ and $\mathrm{supp}(\widehat \mu_{n,x}(\prz))\subseteq\mathrm{Conv}(\law_{n}(\prx))$ almost surely for every $n$;
