...
\begin{document}
	...
	\begin{proposition} \label{prop:sufficient-for-variance-consistency} \linktoproof{proof:prop:sufficient-for-variance-consistency}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				\item $\V_{\law_n}[\prx|\prz] = f(\mu_{n,x}(\prz))$ for $f$ being Lipschitz on domain $\cup_{n=1}^{\infty}\mathrm{Conv}(\mathrm{supp}(\law_{n}(\prx)))$ and $\mathrm{supp}(\widehat \mu_{n,x}(\prz))\subseteq\mathrm{Conv}(\mathrm{supp}(\law_{n}(\prx)))$ almost surely for every $n$;
				\item $\sup_n \E_{\law_n}[|\pry-\mu_{n,y}(\prz)|^{2+\delta}] < \infty$.
			\end{itemize}
		\end{enumerate} 
	\end{proposition}
