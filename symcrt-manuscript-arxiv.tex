...
\begin{document}
	...
	\begin{center}
		\begin{minipage}{\linewidth}
			\begin{algorithm}[H]
				...
				\nonl \textbf{Output:} $\dCRThat$ $p$-value	 \small $\frac{1}{M+1} (1+ \sum_{i=1}^M\indicator\{T_n^{\dCRThat}(\srxk^{(m)}, \srx, \sry, \srz) \geq T_n^{\dCRThat}(\srx, \sry, \srz)\}).$
				\caption{\bf The $\dCRThat$.}
				\label{alg:dcrt-hat}
			\end{algorithm}
		\end{minipage}
	\end{center}
	The hat on $\dCRThat$ is to emphasize that $\law_n(\prx|\prz)$ is approximated rather than known a priori; this is the procedure that is typically applied in practice instead of the dCRT (which required known $\law_n(\prx|\prz)$). The resampled test statistics $T_n^{\dCRThat}(\srxk^{(m)}, \srx, \sry, \srz)$~\eqref{eq:resampled-dcrt-def} have four arguments instead of three in order to emphasize that the conditional mean $\widehat \mu_{n,x}(\cdot)$ is not refit upon resampling.
	...
	\begin{theorem} \label{thm:normal-limit} \linktoproof{proof:thm:normal-limit}
		...
		\begin{gather}
			\P_{\law_n}[(\widehat S_{n}^{\dCRThat})^2 \geq c] \rightarrow 1 \ \text{for some } c > 0
	...
	\begin{proposition} \label{prop:sufficient-for-variance-consistency} \linktoproof{proof:prop:sufficient-for-variance-consistency}
		...
		\begin{enumerate}
			...
			\item $\V_{\lawhat_n}[\prx|\prz] \equiv f(\widehat \mu_{n,x}(\prz))$, if $\V_{\law_n}[\prx|\prz] = f(\mu_{n,x}(\prz))$ for Lipschitz $f$ and \linebreak $\sup_n \E_{\law_n}[|\pry-\mu_{n,y}(\prz)|^{2+\delta}] < \infty$.
		\end{enumerate} 
	\end{proposition}
	
	The first variance estimate given in the proposition can always be applied; the second applies to cases when the mean-variance relationship for $\law_n(\prx|\prz)$ is known and Lipschitz. This is the case, for example, if $\prx$ is binary and we define $f(t) \equiv t(1-t)$.
	...
	\begin{table}[!ht]
		...
		\begin{tabular}{ lllll }
			...
			\hline
			GCM (naive) & intercept-only & all & intercept-only & all/labeled \\
	...
	\begin{itemize}
		\item As one would expect, across all simulation settings, all methods have poorer Type-I error control as sample size $n$ decreases, dimension $p$ increases, sparsity $s$ increases, autocorrelation $\rho$ increases, or confounding strength $\nu$ increases.
	...
	\begin{figure}[h]
		...
		\caption{Type I error control for Gaussian supervised setting: we vary only one parameter in each column and there are five values of the confounding strength $\nu$ in each subplot. Each point is the average of 400 Monte Carlo replicates.}
...
\end{corollary}
Comparing Corollary~\ref{cor:double-robustness-ndcrt} to Corollary~\ref{cor:dcrt-double-robustness}, we see that the $\ndCRThat$ controls Type-I error under weaker assumptions than the $\dCRThat$; in particular the variance of $\law_n(\prx|\prz)$ need not be estimated with any degree of accuracy.
...
\begin{definition} \label{def:conditional-convergence-probability}
	For each $n$, let $W_n$ be a random variable and let $\mathcal F_n$ be a $\sigma$-algebra. Then, we say $W_n$ converges in probability to a constant $c$ conditionally on $\mathcal F_n$ if $W_n$ converges in distribution to the delta mass at $c$ conditionally on $W_n$ (recall Definition~\ref{def:conditional-convergence-distribution}). We denote this convergence by ${W_n \mid \mathcal F_n \convpp c}$. In symbols, 
	\begin{equation}
...
\end{equation*}
On the other hand, the fact that $f$ is Lipschitz (say with Lipschitz constant $L$) gives
\begin{align*}
&\frac{1}{n} \sum_{i=1}^n (f(\widehat \mu_{n,x}(Z_i))-f( \mu_{n,x}(Z_i)))^2\V_{\law_n}[\sry_i\mid \srz_i] \\
&\quad\leq 
\frac{L}{n} \sum_{i=1}^n (\widehat \mu_{n,x}(Z_i)- \mu_{n,x}(Z_i))^2\V_{\law_n}[\sry_i\mid \srz_i] = \frac{1}{L}E'_{n,x} = o_p(1).
\end{align*}
...
$$
The conclusion~\eqref{eq:asymptotic-test-equivalence} of Theorem~\ref{thm:equivalence} implies that $\P_{\law_n}[A_n] \to 0$, so by contiguity of $\law'_n$ to $\law_n$ we also have $\P_{\law'_n}[A_n] \to 0$. This shows the desired asymptotic equivalence of tests~\eqref{eq:equivalent-tests-alternative}. To show the asymptotic equivalence of powers, we derive
\begin{align*}
...
\begin{equation}
	...
	&\quad\leq \sup_n\ \E_{\law_n}[\E_{\law_n}[|\prx - \mu_{n,x}(\prz)|^{1+\delta} \mid \prz]\E_{\law_n}[|\pry - \mu_{n,y}(\prz)|^{1+\delta} \mid \prz]] \\
...
\begin{equation}
	...
	\end{split}
\end{equation}
The convergence $E'_{n,x} \convp 0$ is assumed~\eqref{eq:sp1}. To verify the convergence of the second term, note first that conditional Jensen (Lemma~\ref{lem:conditional-jensen}) and the derivation~\eqref{eq:variance-bound-derivation} give
\begin{equation}
\sup_n\ s^2_n \leq \sup_n\ \E_{\law_n}[|\V_{\law_n}[\prx\mid \prz]\V_{\law_n}[\pry \mid \prz]|^{1+\delta/2}] < \infty,
\end{equation}
...
\begin{lemma}\label{lem:cond_wlln}
	...
	\end{enumerate}
	If we set $S_n\equiv\sum_{i = 1}^n W_{in}$ and $a_n \equiv \sum_{k=1}^n\E[\Bar W_{in}]$ then
	\begin{align*}
...
\begin{proof}
	...
	\begin{align*}
		\P[S_n\neq\Bar{S}_n|\mathcal{F}_n]\leq \P[\cup_{k=1}^n\{\Bar W_{in}\neq W_{in}\}|\mathcal{F}_n] \leq \sum_{k=1}^n\P(|W_{in}|>b_n|\mathcal{F}_n)\convp0
	\end{align*}
	...
	\begin{align*}
		...
		&
		=(b_n\epsilon)^{-2}\sum_{k=1}^n\mathrm{Var}[\Bar W_{in}|\mathcal{F}_n]\\
		&
		\leq (b_n\epsilon)^{-2}\sum_{k=1}^{n}\E\left[\Bar{X}^2_{kn}|\mathcal{F}_n\right]\convp0,
	\end{align*}
