...
\end{equation*}
\nav{ 
The following lemma which which appears as Lemma \ref{lem:conditional-expectation-to-unconditional} is particularly useful now:
\begin{lemma}
...
% \end{proof}
	...
	\new{\section{Effect evaluation of asymmetry nature in $\dCRThat$}}
	It is intuitively true that with similar sample sizes, estimating a Gaussian linear model is easier than a Poisson linear model. 
	With such intuition, we would expect that different resampling strategies will result in different Type-I error control when one 
	considers Gaussian-Poisson model, i.e. $\bm Y|\bm Z$ is a Poisson model and $\bm X|\bm Z$ is a Gaussian model. This is because 
	with fewer sample size it is more difficult to estimate the Poisson model due to the exponential link function. In this section, 
	we provide a set of simulation study to verify such intuition. We consider the following model:
	\begin{align*}
		\bm X|\bm Z\sim N(\bm Z^\top\gamma,1),\ \bm Y|\bm Z\sim \textnormal{Poisson}(\exp(-4+\bm Z^\top\beta)),\ \bm Z\sim N(\bm 0,\bm I_p).
	\end{align*}
	Consider the simulation setting when $n=200$ and $p=10,12,\ldots,18$. The fitting methods applied are GLM for both $\bm X|\bm Z$ and 
	$\bm Y|\bm Z$. Also the unnormalized statistic, $T_n^{\dCRThat}$,  and normalized statistic $T_n^{\ndCRThat}$ are both considered. The 
	simulation is independently repeated $5000$ times. The Type-I error results are presented in Figure \ref{fig:dCRT_GCM_gaussian_poisson_type_I_err}. 
	The most phenomenonal observations are as follows:
	\begin{enumerate}
		\item[(a)] P-values obtaining from resampling from Gaussian model, together with p-value based on $\GCM$, are mcuh more aligned with the significance level;
		\item[(b)] When using normalized statistics, significantly more type-I error appears when resamples are drawn from Poisson model;
		\item[(c)] When using unnormalized statistics, resampling from Poisson model will result in very conservative p-values.
	\end{enumerate}

	It turns out that the performance of unnormalized test statistics observed in (a) and (c) can be explained by the second plot in Figure \ref{fig:dCRT_GCM_gaussian_poisson_histogram}. 
	Notice the unnormalized resampling test statistics are well aligned with sampling test statistics when resmapling from $\lawhat_n(\prx|\prz)$. However, 
	there is an extremely high variation in the resampled test statistics when resampling from $\lawhat_n(\pry|\prz)$. This makes the corresponding p-values very 
	conservative given the comparison between test statistics and resmapling test statistics. 
	
	As for normalized statistics, the resampling variance can make huge impact on the Type-I error control. Specifically, the third plot compares the empirical variance for $\dCRThat$ statistic when resampling from $\lawhat_n(\prx|\prz),\lawhat_n(\pry|\prz)$, which are defined respectively as,
	\begin{align*}
		\frac{1}{n-1}\sum_{i=1}^n \left((\widetilde X_i-\hat\mu_{n,x}(Z_i))(Y_i-\hat\mu_{n,y}(Z_i))-\frac{1}{n}\sum_{i=1}^n(\widetilde X_i-\hat\mu_{n,x}(Z_i))(Y_i-\hat\mu_{n,y}(Z_i)) \right)^2
	\end{align*}
	and 
	\begin{align*}
		\frac{1}{n-1}\sum_{i=1}^n \left((X_i-\hat\mu_{n,x}(Z_i))(\widetilde Y_i-\hat\mu_{n,y}(Z_i))-\frac{1}{n}\sum_{i=1}^n(X_i-\hat\mu_{n,x}(Z_i))(\widetilde Y_i-\hat\mu_{n,y}(Z_i)) \right)^2.
	\end{align*}
	Similarly, we compute the empirical variance for the unnormalized test statistic (so-called sampling empirical variance in the third plot in Figure \ref{fig:dCRT_GCM_gaussian_poisson_histogram})
	\begin{align*}
		\frac{1}{n-1}\sum_{i=1}^n \left((X_i-\hat\mu_{n,x}(Z_i))(Y_i-\hat\mu_{n,y}(Z_i))-\frac{1}{n}\sum_{i=1}^n (X_i-\hat\mu_{n,x}(Z_i))(Y_i-\hat\mu_{n,y}(Z_i))\right)^2,
	\end{align*}
	which serves as the normalization in normalized statistic (up to factor $(n-1)/n$). Clearly, the resampling variance when resampling from $Y|Z$ is drastically large than resampling variance when resampling 
	from $X|Z$. Even though the resampling distribution for unnormalized test statistics in second plot in Figure \ref{fig:dCRT_GCM_gaussian_poisson_histogram} also displays very large variation, this variation 
	has been compensated by the normalization. This causes the observation (b) the absurd multi-modality phenomenon in the frist plot of Figure \ref{fig:dCRT_GCM_gaussian_poisson_histogram}. Moreover, the first plot 
	shows that the range in both histograms of the resampled test statistics from $\lawhat_n(\pry|\prz)$ and $\lawhat_n(\prx|\prz)$ are narrower than the normalied test statistic while the former is more severe. This 
	explains the wild Type-I error behvaior of $\ndCRThat$ when resampling from $\lawhat_n(\pry|\prz)$ (see blue and yellow lines in Figure \ref{fig:dCRT_GCM_gaussian_poisson_type_I_err}). 

	\begin{figure*}
		\centering
		\includegraphics[width=\linewidth]{Figures/varying-dimension-gaussian-poisson-200-10-18.pdf}
		\caption{Type-I error comparison when $\bm Y|\bm Z$ is a $\mathrm{Poisson}$ model.}
		\label{fig:dCRT_GCM_gaussian_poisson_type_I_err} 
	\end{figure*}
	...
	\begin{figure*}
		...
		\label{fig:dCRT_GCM_gaussian_poisson_histogram} 
	\end{figure*}
	We believe that the Type-I error inflation/deflation when resampling from $\lawhat_n(\pry|\prz)$ is due to the amplifying effect of the exponential funciton 
	in the conditional mean $\mu_{y}(\cdot)$. In particular, the exponential function will produce extremely large resamples when the conditional 
	mean happens to be large at some given covariate value. The large variation will make the unnormalized test very conservative and the normalized statistic 
	anti-consevative. Therefore, the asymmetry nature in $\dCRThat$ can be a double-edged sword depending on the underlying data generating mechanism.
\end{document}
